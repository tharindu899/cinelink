...
function EpisodeCard({ ep, onRequestEpisode }) {
  ...
  return (
    <div className={`glass rounded-xl p-4 border transition-all duration-200 ${
      ep.custom_link ? 'border-brand-500/30 hover:border-brand-500/60' : 'border-white/5 hover:border-white/10'
    }`}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <span className="font-mono text-xs font-semibold text-brand-400">{label}</span>
        <span className={`w-1.5 h-1.5 rounded-full mt-1 flex-shrink-0 ${ep.custom_link ? 'bg-green-400' : 'bg-white/20'}`} />
      </div>
      ...
      {ep.custom_link ? (
        <a href={ep.custom_link} target="_blank" rel="noreferrer noopener"
          ...
          <FiPlay size={11} className="fill-white" /> Watch <FiExternalLink size={10} />
        </a>
      ) : (
...
export default function SeriesDetails() {
  ...
  return (
    <div className="min-h-screen pb-16">
      ...
      {/* ── HERO: backdrop + horizontal poster | info ── */}
          ...
          <div className="flex flex-row gap-7 sm:gap-10 items-start">
            ...
            <div className="flex-1 min-w-0 space-y-3 sm:space-y-4 pt-1 sm:pt-3">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 pt-1">
                {fbData?.custom_link ? (
                  <a href={fbData.custom_link} target="_blank" rel="noreferrer noopener" className="btn-brand text-sm">
                    <FiPlay className="fill-white" size={13} /> Watch Series <FiExternalLink size={11} />
                  </a>
                ) : (
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 space-y-14 mt-10">
        ...
        {fbData && (
          <div className="glass rounded-2xl p-5 border-l-4 border-brand-500 space-y-3">
            ...
            {fbData.custom_link && (
              <a href={fbData.custom_link} target="_blank" rel="noreferrer noopener" className="btn-brand inline-flex text-sm">
                <FiPlay className="fill-white" size={13} /> Watch Series <FiExternalLink size={11} />
              </a>
            )}
          </div>
