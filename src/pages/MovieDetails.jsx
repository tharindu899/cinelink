...
export default function MovieDetails() {
  ...
  return (
    <div className="min-h-screen pb-16">
        ...
        <div className="relative z-10 max-w-screen-xl mx-auto px-4 sm:px-6 pt-28 pb-14">
          ...
          <div className="flex flex-row gap-7 sm:gap-10 items-start">
            ...
            <div className="flex-1 min-w-0 space-y-3 sm:space-y-4 pt-1 sm:pt-3">
              ...
              {movie.tagline && (
                <p className="text-white/40 font-body italic text-xs sm:text-sm">&ldquo;{movie.tagline}&rdquo;</p>
              )}

              {/* Meta */}
              <div className="flex flex-wrap items-center gap-2">
                {movie.vote_average > 0 && (
                  <span className="rating-badge"><FiStar className="fill-amber-400" size={10} />{movie.vote_average.toFixed(1)}</span>
                )}
                {year && <span className="flex items-center gap-1 text-white/50 text-xs sm:text-sm"><FiCalendar size={11} /> {year}</span>}
                {runtime && <span className="flex items-center gap-1 text-white/50 text-xs sm:text-sm"><FiClock size={11} /> {runtime}</span>}
                {movie.original_language && <span className="tag uppercase text-xs">{movie.original_language}</span>}
              </div>
        ...
        {fbData && qualityLinks && (
          <div className="glass rounded-2xl p-5 border-l-4 border-brand-500 space-y-4">
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-white/55">
              {addedDate && (
                <span className="flex items-center gap-1.5"><FiCalendar size={12} className="text-brand-400" /> Added {addedDate}</span>
              )}
              {fbData.note && (
      ...
      {showReq && <RequestModal item={movie} type="movie" onClose={() => setShowReq(false)} />}
    </div>
  );
}
