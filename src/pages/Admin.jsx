...
import {
  ...
  FiX, FiCheck, FiRefreshCw, FiFilm, FiTv,
...
export default function Admin() {
  ...
  const [editItem,   setEditItem]   = useState(null); // { tmdbData, fbData }
                    ...
                    return (
                      ...
                      >
                        {poster ? (
                          ...
                          <div className="w-8 h-12 bg-dark-700 rounded flex-shrink-0 flex items-center justify-center text-base">
                            {isTV ? '📺' : '🎬'}
                          </div>
                        )}
        ...
        {tab === 'requests' && (
          <div className="space-y-3">
            {requests.length === 0 ? (
              ...
              requests.map(req => (
                <RequestRow key={req.id} req={req} onAction={handleRequestAction} />
              ))
            )}
          </div>
        )}
    ...
    <div className="glass rounded-xl p-4 flex items-center gap-4 border border-white/5 hover:border-white/10 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-white/40 text-xs mt-0.5">
          ...
          {entry.note && <span className="ml-2 text-blue-400">· Has note</span>}
        </p>
      </div>

      <div className="flex items-center gap-2 flex-shrink-0">
        <button onClick={() => onEdit(entry)} className="btn-ghost p-2 text-blue-400 hover:text-blue-300">
          <FiEdit2 size={14} />
        </button>
        <button onClick={() => onDelete(entry)} className="btn-ghost p-2 text-red-400 hover:text-red-300">
          <FiTrash2 size={14} />
        </button>
      </div>
    </div>
...
function RequestRow({ req, onAction }) {
  ...
  return (
    <div className="glass rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-4 border border-white/5">
      {req.poster_path && (
        <img
      ...
      {req.status === 'pending' && (
        <div className="flex gap-2 flex-shrink-0">
          <button
            onClick={() => onAction(req, 'fulfilled')}
            className="btn-ghost py-1.5 px-3 text-xs text-green-400 hover:text-green-300"
          >
            <FiCheckCircle size={13} /> Fulfill
          </button>
          <button
...
function EntryFormModal({ item, onClose }) {
  ...
  const [note,    setNote]    = useState(fbData?.note || '');
    ...
    >
      ...
      <div className="relative glass rounded-2xl p-6 w-full max-w-md shadow-2xl shadow-black/60 animate-fade-up">
        ...
        <div className="flex items-center gap-3 p-3 rounded-xl bg-dark-700 border border-white/10 mb-6">
          {poster && (
            <img src={poster} alt={title} className="w-12 h-16 object-cover rounded-lg flex-shrink-0" />
          )}
          <div>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="e.g. HD quality, English subtitles…"
              rows={3}
